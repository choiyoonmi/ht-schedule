  ...
  for (const st of students) {
    for (const subj in st.selectedTeachers) {
      for (const e of st.selectedTeachers[subj]) {
        ...
        const g = st.name === '김주원(5)' && subj === '초등영어'
          ? 4
          : (st.name === '홍리아' && subj === '초등영어') ? 5 : isMixedEnglishStudent ? 2 : st.grade; // 합반 보정
...
function migrateScheduleCorrections(students: Student[]): Student[] {
  ...
  const specialGrade3Schedule = (): Student['selectedTeachers'] => ({
    ...
    국어: [
      { teacherId: 'korean_1', day: '월', hour: 15 },
      { teacherId: 'korean_1', day: '수', hour: 16 },
    ],
    숙제반: [
  ...
  const grade3EnglishSchedules: Record<string, TeacherSelection[]> = {
    ...
    '초등4 1반': [
      ...fiveDayEnglish(['elem_eng_1', 'elem_eng_2', 'elem_eng_1', 'elem_eng_2', 'elem_eng_1'], 16),
    ],
    '초등4 2반': [
      ...fiveDayEnglish(['elem_eng_3', 'elem_eng_1', 'elem_eng_3', 'elem_eng_1', 'elem_eng_3'], 16),
    ],
    '초등 파닉스2반': [
      { teacherId: 'elem_eng_1', day: '월', hour: 16 },
      { teacherId: 'elem_eng_1', day: '화', hour: 14 },
      { teacherId: 'elem_eng_2', day: '수', hour: 16 },
      { teacherId: 'elem_eng_1', day: '목', hour: 16 },
  ...
  return students
    ...
    if (student.grade === 3 && englishClass) {
      if (student.name === '박수현(3)') {
        const reference = students.find(item => item.name === '김시안');
        student = {
          ...student,
          selectedTeachers: {
            ...student.selectedTeachers,
            초등수학: student.selectedTeachers['초등수학']?.length
              ? student.selectedTeachers['초등수학']
              : (reference?.selectedTeachers['초등수학'] || []).map(item => ({ ...item })),
            국어: student.selectedTeachers['국어']?.length
              ? student.selectedTeachers['국어']
              : (reference?.selectedTeachers['국어'] || []).map(item => ({ ...item })),
          },
        };
      }
      const englishSchedule = grade3EnglishSchedules[englishClass];
      const isEnglish4Class = englishClass === '초등4 1반' || englishClass === '초등4 2반';
      const isPhonics2 = englishClass === '초등 파닉스2반';
      const math: TeacherSelection[] = isEnglish4Class
        ? [
            { teacherId: 'elem_math_2', day: '월', hour: 14 },
            { teacherId: 'elem_math_1', day: '화', hour: 15 },
            { teacherId: 'elem_math_1', day: '수', hour: 14 },
            { teacherId: 'elem_math_2', day: '금', hour: 14 },
            { teacherId: 'elem_math_1', day: '금', hour: 15 },
          ]
        : isPhonics2
        ? [
            { teacherId: 'elem_math_2', day: '월', hour: 14 },
      ...
      const korean: TeacherSelection[] = isEnglish4Class
        ? [
            { teacherId: 'korean_1', day: '화', hour: 14 },
            { teacherId: 'korean_1', day: '수', hour: 15 },
          ]
        : isPhonics2
        ? [
            { teacherId: 'korean_1', day: '화', hour: 16 },
            { teacherId: 'korean_1', day: '수', hour: 15 },
          ]
        : [
            { teacherId: 'korean_1', day: '화', hour: 14 },
      ...
      const homework: TeacherSelection[] = isEnglish4Class || isPhonics2
        ? [
        ...
        : [
            ...
            { teacherId: '', day: '금', hour: 16 },
          ];
      return {
        ...student,
        englishClass,
        selectedTeachers: englishSchedule
          ? { ...student.selectedTeachers, 초등수학: math, 초등영어: englishSchedule, 국어: korean, 숙제반: homework }
          : student.selectedTeachers,
      };
    }
    ...
    if (student.name === '배소이' || student.name === '이준희') {
      const homework = (student.selectedTeachers['숙제반'] || []).filter(selection =>
        ...
        !(selection.day === '금' && selection.hour === 15)
      );
      const hasThursdayHomework15 = homework.some(selection => selection.day === '목' && selection.hour === 15);
      ...
      const completedHomework = [
        ...homework,
        ...(hasThursdayHomework15 ? [] : [{ teacherId: '', day: '목' as DayOfWeek, hour: 15 }]),
      ];
      return {
        ...student,
        englishClass: '초등3 1반',
        selectedTeachers: {
          ...
          // 초2도 17시에 하원하므로 17:00 행에는 수업을 두지 않는다.
          // 영어·국어는 그대로 두고 수학 한 칸만 화요일 15시로 이동한다.
          초등수학: [
            { teacherId: 'elem_math_2', day: '화', hour: 15 },
            { teacherId: 'elem_math_2', day: '화', hour: 16 },
            { teacherId: 'elem_math_2', day: '목', hour: 16 },
            { teacherId: 'elem_math_2', day: '금', hour: 15 },
          ],
          국어: korean,
    ...
    if (student.grade === 1) {
      const isClassOne = ['김재헌', '정예린'].includes(student.name);
      return {
        ...student,
        selectedTeachers: {
          ...student.selectedTeachers,
          // 첨부된 기존표의 영어·국어 시간은 그대로 복원한다.
          초등영어: isClassOne
            ? [
                { teacherId: 'elem_eng_1', day: '월', hour: 15 },
                { teacherId: 'elem_eng_2', day: '화', hour: 15 },
                { teacherId: 'elem_eng_1', day: '수', hour: 15 },
                { teacherId: 'elem_eng_2', day: '금', hour: 15 },
              ]
            : [
                { teacherId: 'elem_eng_3', day: '월', hour: 16 },
                { teacherId: 'elem_eng_2', day: '화', hour: 15 },
                { teacherId: 'elem_eng_2', day: '수', hour: 16 },
                { teacherId: 'elem_eng_3', day: '금', hour: 15 },
              ],
          국어: [
            { teacherId: 'korean_1', day: '화', hour: 16 },
            { teacherId: 'korean_1', day: '목', hour: 16 },
          ],
          // 영어·국어의 빈 칸에만 수학을 넣고 두 반을 분리한다.
          초등수학: isClassOne
            ? [
                { teacherId: 'elem_math_2', day: '월', hour: 16 },
                { teacherId: 'elem_math_1', day: '수', hour: 16 },
                { teacherId: 'elem_math_1', day: '목', hour: 15 },
                { teacherId: 'elem_math_1', day: '금', hour: 16 },
              ]
            : [
                { teacherId: 'elem_math_1', day: '월', hour: 15 },
                { teacherId: 'elem_math_1', day: '수', hour: 15 },
                { teacherId: 'elem_math_1', day: '목', hour: 15 },
                { teacherId: 'elem_math_1', day: '금', hour: 16 },
              ],
          숙제반: [],
    ...
    if (student.grade === 4) {
      if (student.name === '홍리아' && !(student.selectedTeachers['국어'] || []).length) {
        const reference = students.find(item => item.name === '김민채');
        student = {
          ...student,
          selectedTeachers: {
            ...student.selectedTeachers,
            국어: (reference?.selectedTeachers['국어'] || []).map(item => ({ ...item })),
          },
        };
      }
      const homework = (student.selectedTeachers['숙제반'] || []).filter(selection => !(selection.day === '화' && (selection.hour === 14 || selection.hour === 15)));
      return {
        ...student,
        englishClass: undefined,
        selectedTeachers: {
          ...student.selectedTeachers,
          초등수학: (student.selectedTeachers['초등수학'] || []).map(selection =>
            selection.day === '화' && selection.hour === 15
              ? { ...selection, hour: 14, teacherId: 'elem_math_1' }
              : selection.day === '월' && selection.hour === 15
                ? { ...selection, teacherId: 'elem_math_2' }
              : selection.day === '금' && selection.hour === 15
                ? { ...selection, hour: 14, teacherId: 'elem_math_1' }
                : selection
          ),
          국어: (student.selectedTeachers['국어'] || []).map(selection =>
            (selection.day === '수' || selection.day === '목') && selection.hour === 14
              ? { ...selection, day: '화', hour: 15 }
              : selection
          ),
          숙제반: [
            ...homework.filter(selection => !(
              (selection.day === '금' && selection.hour === 14) ||
              (selection.day === '금' && selection.hour === 15)
            )),
            { teacherId: '', day: '금', hour: 15 },
          ],
    ...
    if (student.name === '김주원(5)') {
      const englishReference = students.find(item => item.name === '위지유');
      const grade5Reference = students.find(item => item.name === '김의진');
      return {
        ...student,
        englishClass: undefined,
        selectedTeachers: {
          ...student.selectedTeachers,
          초등영어: (englishReference?.selectedTeachers['초등영어'] || []).map(item => ({ ...item })),
          초등수학: (grade5Reference?.selectedTeachers['초등수학'] || []).map(item =>
            item.day === '수' && (item.hour === 14 || item.hour === 15)
              ? { ...item, hour: 17, teacherId: 'elem_math_2' }
              : { ...item }
          ),
          국어: (grade5Reference?.selectedTeachers['국어'] || []).map(item => ({ ...item })),
          숙제반: [
            ...(student.selectedTeachers['숙제반'] || []).filter(item => !(item.day === '수' && item.hour === 15)),
            { teacherId: '', day: '수', hour: 15 },
          ],
        },
      };
    ...
    return {
      ...
      selectedTeachers: {
        ...student.selectedTeachers,
        초등수학: (student.selectedTeachers['초등수학'] || []).map(selection =>
          selection.day === '수' && (selection.hour === 14 || selection.hour === 15)
            ? { ...selection, hour: 17, teacherId: 'elem_math_2' }
            : selection
        ),
        국어: (student.selectedTeachers['국어'] || []).map(selection =>
        ...
        숙제반: [
          ...(student.selectedTeachers['숙제반'] || []).filter(selection => !(selection.day === '수' && selection.hour === 15)),
          { teacherId: '', day: '수', hour: 15 },
        ],
      },
    };
...
const SEED_VERSION = '2026-08-2학기-블록v16-math-teacher-alternating';
